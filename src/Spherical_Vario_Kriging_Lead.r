...
model.variog <- vgm(psill=500, model="Sph", nugget=50, range=0.4) print(fit.variog <- fit.variogram(variog1, model.variog)) plot(variog1, model = fit.variog,pch=20,col="red" ) 
 
 
LeadOKrig= krige(Pb~1, spat.samp, spat.grid, model=fit.variog) 
 
plot(LeadOKrig, main="Spherical Surface by Kriging") 
