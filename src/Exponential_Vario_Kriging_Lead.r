...
# construct a grid of locations to predict at grid <- expand.grid(x=seq(min(sample[,1], na.rm=T),max(sample[,1], na.rm=T), 0.001), y=seq(min(sample[,2], na.rm=T),max(sample[,2], na.rm=T), 0.001)) plot(grid) spat.grid <- grid 
...
model.variog <- vgm(psill=500, model="Exp", nugget=50, range=0.25) print(fit.variog <- fit.variogram(variog1, model.variog)) plot(variog1, model = fit.variog,pch=20,col="darkgreen" ,main="Exponential Model") 
 
 
LeadOKrig= krige(Pb~1, spat.samp, spat.grid, model=fit.variog) 
 
plot(LeadOKrig, main="Exponential Surface by Kriging") 
